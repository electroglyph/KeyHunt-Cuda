...
# Features
- Bitcoin P2PKH address search (hash160) with mode `-m ADDRESS` or `-m ADDRESSES`
- XPoint[s] mode for searching by x-coordinate with mode `-m XPOINT` or `-m XPOINTS`
- For XPoint[s] mode use x point of the public key, without 02 or 03 prefix (64 chars).
- Cuda only (CPU + GPU separate modes).

# Usage
- For multiple addresses or xpoints, file format must be binary with sorted data.
- To convert Bitcoin addresses list (text format) to hash160 hashes binary file use provided python script `addresses_to_hash160.py`
...
## BinSort
For hash160 `length` is `20` and for xpoint `length` is `32`.
```
BinSort.exe
